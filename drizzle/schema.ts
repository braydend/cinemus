import { mysqlTable, mysqlSchema, AnyMySqlColumn, primaryKey, unique, varchar, text, int, datetime, index, mysqlEnum, tinyint } from "drizzle-orm/mysql-core"
...
export const listMember = mysqlTable("ListMember", {
	userId: varchar("userId", { length: 191 }).notNull(),
	listId: varchar("listId", { length: 191 }).notNull(),
	role: mysqlEnum("role", ['VIEWER','COLLABORATOR','MODERATOR']).default('VIEWER').notNull(),
},
(table) => {
	return {
...
export const user = mysqlTable("User", {
	...
	role: mysqlEnum("role", ['ADMIN','STANDARD']).default('STANDARD').notNull(),
},
(table) => {
	return {
