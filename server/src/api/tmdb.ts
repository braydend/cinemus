...
const get = async <MediaType extends Media>(id: string, type: MediaCategory): Promise<MediaType> => {
    try {
        const response = await axios.get<MediaType>(`${tmdbBaseUrl}/${type}/${id}?api_key=${secrets.MOVIE_DB_API_KEY}`);
